...
C  Get the positions, velocities, and atom types
        call getx(x,v,lbox,hlbox,natom,frst)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine getx(x,v,lbox,hlbox,natom,frst)
        integer nmax
        parameter(nmax=2000)
...
C$OMP&          ,mass,umblist,rumb,kumb1,frst2,frst1,mtot1,mtot2
C$OMP&          ,e0,p0,q0,o0,rmax,eps,rcom)
...
C  Coulomb
              ...
              fij(1)=fs*(dp1*dx/r0-enow(1))
              fij(2)=fs*(dp1*dy/r0-enow(2))
              fij(3)=fs*(dp1*dz/r0-enow(3))
              fs=-charge(j)*q0*(1.5d0*c2-0.5d0)/r2**2
              fij(1)=fij(1)+fs*(dp2*dx/r0-dp1*enow(1))
              fij(2)=fij(2)+fs*(dp2*dy/r0-dp1*enow(2))
              fij(3)=fij(3)+fs*(dp2*dz/r0-dp1*enow(3))
              fs=-charge(j)*o0*(2.5d0*c3-1.5d0*c1)/r2**2/r0
              fij(1)=fij(1)+fs*(dp3*dx/r0-dp2*enow(1))
              fij(2)=fij(2)+fs*(dp3*dy/r0-dp2*enow(2))
              fij(3)=fij(3)+fs*(dp3*dz/r0-dp2*enow(3))
C  LJ
              if(r0.lt.rmax) then
                dr=r0/0.1d0+0.5d0
                ir=int(dr)
...
C
        ...
        open(20,FILE='sample.crd',STATUS='old')
        read(20,*) it,nstyp
        do i=1,it
          read(20,*) j
          styp(i)=j
        ...
        close(20)
C
        open(20,FILE='sample.SPA2.dat',STATUS='old')
        do i=1,nstyp
          do j=1,hmax
            read(20,*) r0,g0,it,k
            if(k.le.0) then
        ...
        close(20)
C
        open(20,FILE='sample.pol2.dat',STATUS='old')
        do i=1,nstyp
          it=ttyp(i)
          do j=1,hmax
            read(20,*) r0,g0
        ...
        close(20)
C     
        open(20,FILE='sample.frc.dat',STATUS='old')
        do i=1,nstyp
          do j=1,hmax
            read(20,*) r0,g0
            fLJ(j,i)=g0
